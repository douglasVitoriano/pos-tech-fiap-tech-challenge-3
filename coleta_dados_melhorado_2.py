...
def train(df):
    ...
    df = pd.get_dummies(df, columns=["city"], drop_first=True)  # One-hot encoding for 'city'
    ...
    df.to_csv("real_estate_data_trained.csv", index=False)
...
def predict():
    try:
        # Input data
        df = preprocess_data()
        input_processed = train(df)
        
        # Ensure the columns match
        missing_cols = set(input_processed.X.columns) - set(input_processed.columns)
